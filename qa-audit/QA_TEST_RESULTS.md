...
| `npm test` | Unit/security/subscription regressions | All tests pass | 117/117 passed | PASS | Local audit run |
...
| `npm run test:rules` | Firestore/Storage integration | Emulator rules tests | Blocked: Java <21 | BLOCKED | Firebase CLI output |
...
Notable automated coverage already present includes account deletion, Stripe/webhook idempotency, subscription seats and downgrade behavior, server authorization, input bounds, secure rich text/RSS processing, public volunteer/fundraising, and access-control regressions.
