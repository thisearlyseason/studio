...
import { Trophy, Plus, MapPin, Calendar, TrendingUp, TrendingDown, MinusCircle, Edit2, Lock, Sparkles, LineChart as ChartIcon, ChevronRight, Zap, Quote, Shield, Info, Loader2 } from 'lucide-react';
...
export default function GamesPage() {
  ...
  const handleRecordGame = () => {
    ...
    if (selectedLeagueId !== 'none' && selectedOpponentTeamId !== 'manual') {
  ...
  const resetForm = () => { 
    setOpponent(''); setDate(''); setMyScore(''); setOpponentScore(''); setLocation(''); setNotes(''); setEditingGame(null); 
    setSelectedLeagueId('none'); setSelectedOpponentTeamId('manual');
  };
  ...
  const handleEditGame = (game: any) => {
    ...
    setSelectedLeagueId(game.leagueId || 'none');
    setSelectedOpponentTeamId(game.opponentTeamId || 'manual');
                    ...
                    <div className="space-y-4">
                      {leagueOpponents.length > 0 && (
                        <div className="space-y-1.5">
                          ...
                          <p className="text-[8px] font-bold text-muted-foreground uppercase px-1 italic">Selecting a league opponent updates competitive standings automatically.</p>
                        </div>
                      )}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input 
                          ...
                          disabled={selectedOpponentTeamId !== 'manual'}
                          className="rounded-xl h-11 lg:h-12 border-2 font-black text-sm" 
                        />
                      </div>
                      <div className="space-y-1.5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      {isPro ? (
                        <Textarea 
                          ...
                          className="min-h-[150px] lg:min-h-[200px] rounded-2xl lg:rounded-[2rem] p-4 lg:p-6 font-bold bg-muted/10 border-2 resize-none text-sm" 
                        />
                      ) : (
                        <div className="min-h-[150px] lg:min-h-[200px] rounded-2xl lg:rounded-[2rem] p-6 bg-primary/5 border-2 border-dashed flex flex-col items-center justify-center text-center space-y-3">
                          ...
                          <Button size="sm" variant="ghost" className="h-7 text-[8px] font-black uppercase text-primary" onClick={purchasePro}>Upgrade to Elite</Button>
                        </div>
                      )}
                    </div>
      ...
      {isPro ? (
        ...
        <div className="bg-muted/30 p-8 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center text-center space-y-4">
          <div className="bg-white p-4 rounded-2xl shadow-sm relative">
            <ChartIcon className="h-8 w-8 text-primary/40" />
            <Lock className="absolute -top-1 -right-1 h-4 w-4 bg-black text-white p-0.5 rounded-full border-2 border-background" />
          </div>
          <div className="space-y-1">
            <h3 className="text-sm font-black uppercase tracking-tight">Performance Analytics Locked</h3>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest max-w-xs">Upgrade to Pro to visualize season trajectory, momentum trends, and win/loss metrics.</p>
          </div>
          <Button size="sm" className="rounded-full h-9 px-6 font-black uppercase text-[10px] tracking-widest" onClick={purchasePro}>Unlock Pro Analytics</Button>
        </div>
      )}
            ...
            <Card className="rounded-3xl lg:rounded-[2.5rem] border-none shadow-xl ring-1 ring-black/5 overflow-hidden bg-white">
              <CardHeader className="bg-muted/30 border-b flex flex-row items-center justify-between px-6 lg:px-8 py-4 lg:py-6">
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle className="text-[10px] lg:sm font-black uppercase tracking-[0.2em]">Trajectory</CardTitle>
                </div>
              </CardHeader>
                  ...
                  <CardContent className="p-4 lg:p-6 space-y-3 lg:space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex gap-2">
                        ...
                        {game.leagueId && <Badge className="bg-primary/10 text-primary border-none text-[8px] px-2 h-5 font-black uppercase flex items-center gap-1"><Shield className="h-2 w-2" /> League</Badge>}
                      </div>
                    ...
                    {isPro && game.notes && <p className="text-[9px] lg:text-[11px] font-medium text-muted-foreground line-clamp-1 italic border-t pt-2 lg:pt-3">"{game.notes}"</p>}
                  </CardContent>
                </Card>
              ))}
            </div>
