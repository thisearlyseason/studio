        ...
        <Card className="max-w-lg w-full p-10 rounded-[3rem] border-none shadow-2xl bg-white animate-in zoom-in-95 duration-500">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-black text-white p-8 rounded-3xl space-y-4 text-left shadow-xl group">
              ...
              <Button asChild className="w-full h-12 rounded-xl bg-white text-black hover:bg-primary hover:text-white transition-all font-black uppercase text-[10px] tracking-widest border-none">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30 p-6">
        <Card className="max-w-md w-full text-center p-10 rounded-[3rem] border-none shadow-2xl">
          ...
          <h2 className="text-2xl font-black uppercase tracking-tight text-foreground">Portal Inactive</h2>
          ...
          <form onSubmit={handleSubmit}>
            <CardHeader className="p-8 lg:p-10 pb-4">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <CardTitle className="text-2xl font-black uppercase tracking-tight leading-none text-foreground">Enrollment Data</CardTitle>
            ...
            <CardContent className="p-8 lg:p-10 space-y-8">
                ...
                <div key={field.id} className="space-y-3">
                  {field.type === 'header' ? (
                    <div className="pt-4 border-b pb-2">
                      <h3 className="font-black text-lg uppercase tracking-tight text-foreground">{field.label}</h3>
                    ...
                    <>
                      ...
                      {field.type === 'short_text' && (
                        <Input 
                          ...
                          className="h-12 rounded-xl border-2 font-bold bg-muted/10 focus:bg-white transition-all text-foreground"
                        />
                      )}
                      ...
                      {field.type === 'long_text' && (
                        <Textarea 
                          ...
                          value={answers[field.id] || ''} 
                          onChange={e => handleInputChange(field.id, e.target.value)}
                          className="rounded-xl min-h-[100px] border-2 font-medium bg-muted/10 focus:bg-white transition-all text-foreground"
                        />
                      )}
                      ...
                      {field.type === 'dropdown' && (
                        ...
                        >
                          <SelectTrigger className="h-12 rounded-xl border-2 font-bold bg-muted/10 text-foreground"><SelectValue placeholder="Select choice..." /></SelectTrigger>
                          <SelectContent className="rounded-xl">
                            {field.options?.map(opt => <SelectItem key={opt} value={opt} className="font-bold text-foreground">{opt}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      )}
                      ...
                      {field.type === 'yes_no' && (
                        ...
                        >
                          <div className="flex items-center space-x-2">
                            ...
                            <Label htmlFor={`${field.id}_yes`} className="font-black text-xs uppercase cursor-pointer text-foreground">Yes</Label>
                          </div>
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value="no" id={`${field.id}_no`} />
                            <Label htmlFor={`${field.id}_no`} className="font-black text-xs uppercase cursor-pointer text-foreground">No</Label>
                          </div>
                        </RadioGroup>
                      )}
                          ...
                          {field.options?.map(opt => {
                            ...
                            return (
                              <div key={opt} className="flex items-center space-x-3">
                                ...
                                />
                                <Label htmlFor={`${field.id}_${opt}`} className="text-[10px] font-black uppercase tracking-tight cursor-pointer text-foreground">{opt}</Label>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      {field.type === 'image' && (
                        <div className="p-10 border-2 border-dashed rounded-[2rem] bg-muted/5 text-center space-y-4 group cursor-pointer hover:border-primary/20 transition-all relative text-foreground">
                          <input 
                            type="file" 
                            accept="image/*" 
              ...
              {config.waiver_text && (
                <div className="space-y-6 pt-8 border-t">
                  <div className="flex items-center gap-3"><Signature className="h-6 w-6 text-primary" /><h4 className="text-lg font-black uppercase tracking-tight text-foreground">Institutional Waiver</h4></div>
                  <ScrollArea className="h-40 p-4 rounded-2xl bg-muted/10 border-2 font-medium text-xs leading-relaxed text-foreground">
                    {config.waiver_text}
                  </ScrollArea>
                  <div className="flex items-center space-x-3 p-4 bg-primary/5 rounded-2xl border border-primary/10 group cursor-pointer" onClick={() => setWaiverAgreed(!waiverAgreed)}>
                    ...
                    <Label htmlFor="waiver_agree" className="text-[10px] font-black uppercase tracking-tight cursor-pointer leading-tight text-foreground">
                      I verify that I have read and accept all participation terms.
                    </Label>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest ml-1 text-foreground">Digital Signature (Legal Name)</Label>
                    <Input 
                      placeholder="Type your name to sign..." 
                      value={signature} 
                      onChange={e => setSignature(e.target.value)} 
                      className="h-14 rounded-xl border-2 font-mono italic text-center text-xl bg-muted/10 focus:bg-white text-foreground" 
                      required
                    />
                  </div>
                </div>
            ...
            <CardFooter className="p-8 lg:p-10 pt-0">
              <Button 
                type="submit" 
                className="w-full h-16 rounded-2xl text-lg font-black shadow-xl shadow-primary/20 active:scale-95 transition-all border-none"
